...
@router.post("/secretary/call-next")
async def call_next_patient_for_secretary(
    current_user: User = Depends(get_admin_or_staff_user),
    db: Session = Depends(get_db)
):
    """Call the next patient for the secretary's assigned service."""
    try:
        # Check if user has an assigned service
        if not current_user.assigned_service_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No service assigned to this user"
            )
        
        # Import the atomic function from queue router
        from routers.queue import _call_next_patient_atomic
        
        # Call the next patient using the atomic function
        result = await _call_next_patient_atomic(current_user.assigned_service_id, db, current_user)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error calling next patient: {str(e)}"
        ) 