    ...
    enhanceMessageWithStats(message) {
        ...
        const contextualMessages = {
            "Résumez l'activité d'aujourd'hui": 
                `Résumez l'activité d'aujourd'hui. Contexte actuel: ${this.systemStats.today_tickets} tickets créés, ${this.systemStats.total_waiting} patients en attente, temps moyen ${this.systemStats.avg_wait_time} minutes, ${this.systemStats.active_services} services actifs (incluant les urgences).`,
            
            "Quels services ont le plus d'attente ?": 
                `Analysez les services avec le plus d'attente. Il y a actuellement ${this.systemStats.total_waiting} patients en attente avec un temps moyen de ${this.systemStats.avg_wait_time} minutes sur ${this.systemStats.active_services} services (actifs + urgences).`,
            
            "Comment optimiser les temps d'attente ?": 
                `Proposez des optimisations pour les temps d'attente. Situation actuelle: ${this.systemStats.avg_wait_time} minutes en moyenne, ${this.systemStats.active_services} services actifs (incluant les urgences).`
        };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.chatbot = window.adminChatbot; // Also set as chatbot for compatibility
